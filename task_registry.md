...
| `global-multisource-mainline` | `全球多源主线验收标准重定标 + 报告分层` | `active` | `main` | `2026-04-22` | `sed -n '1,260p' src/news_sentiment/reporting/text_report.py` | `不要再按旧 phase8 的 A股单线标准验收；先做分层总榜，不要先改 event_merge/analysis；低信号先看模板材料/栏目稿/进展包，不要把海外/宏观样本一刀切当噪音` |
