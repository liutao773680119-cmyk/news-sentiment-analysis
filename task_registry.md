...
| `global-multisource-mainline` | `全球多源主线验收标准重定标 + 报告分层` | `active` | `main` | `2026-04-27` | `PYTHONPATH=src python -m news_sentiment audit-suspicious --limit 10` | `不要再按旧 phase8 的 A股单线标准验收；当前 report 头部已主要剩真实风险/并购样本；不要继续为了更干净机械扩词表；如要补原样 pytest，先处理当前 Windows Python 的代理/镜像问题` |
