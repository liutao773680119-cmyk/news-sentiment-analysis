...
| `global-multisource-mainline` | `全球多源主线 live report 尾噪收口` | `active` | `main` | `2026-05-11` | `sed -n '1,180p' data/reports/latest_report.txt` | `不要再按旧 phase8 的 A股单线标准验收；audit=0 后仍要直接看 report 头部；report 过滤和 audit-suspicious 要同步；report 重写和读取不要并行；不要查错 analyses 路径，当前是 data/events/event_analysis.jsonl；不要硬绑旧 event_id 排 live 问题；cninfo 年报问询函和发行优先股/诉讼材料先走最窄 title-only 口径，不扩题材` |
