...
|---|---|---|---|---|---|---|
| `phase8-live-boundary` | `live 样本边界收口 + irm 问答/股权激励材料/cls 栏目稿降噪` | `active` | `main` | `2026-04-20` | `PYTHONPATH=src ./.venv/bin/python -m news_sentiment live-smoke --source all` | `先看当天 report 头部和 audit-suspicious；不沿用上一轮头部清单；头部如果已回到退市风险类公告先停；不要把当前 legit 风险公告继续硬压；不要把 hkex staged 和主线混写；不要为追求全净继续过拟合 text_report` |
