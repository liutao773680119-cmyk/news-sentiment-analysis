...
| `global-multisource-mainline` | `全球多源主线 live report 尾噪收口` | `active` | `main` | `2026-05-20` | `sed -n '/## Report Highlights/,+80p' data/monitoring/summaries/$(ls -1 data/monitoring/summaries/*-summary.md | tail -1)` | `不要再按旧 phase8 的 A股单线标准验收；audit=0 后仍要直接看 report 头部；Report Highlights 是 latest_report 摘要入口，不是完整报告或历史回放；report 过滤和 audit-suspicious 要同步；report 重写和读取不要并行；不要查错 analyses 路径，当前是 data/events/event_analysis.jsonl；不要硬绑旧 event_id 排 live 问题；cninfo 年报问询函和发行优先股/诉讼材料先走最窄 title-only 口径，不扩题材；外盘指数+题材板块联动若有 A股映射，按 market_reference 保留 report，仅从 audit 异常口径排除；A股概念走强/活跃或板块震荡走强且带涨停/连板/创新高/大涨/涨幅居前上下文时也按 market_reference 处理，不当噪音删除；港股主题股异动且有明确A股题材映射时也按 market_reference 处理；短标题诉讼/仲裁材料只按 audit 尾噪处理；募集资金账户被冻结、股东所持部分股份冻结按用户口径视作 audit 尾噪；会见/交流类只有在无签署/中标/订单/合同/采购时才按尾噪；现货黄金/白银点位和涨跌幅播报按尾噪，不扩到黄金产业链新闻；watchdog 6 小时汇总只生成报告，不自动改规则；淘金互动易算力产业链按 market_reference 保留报告；私营生物医药C轮融资泛稿只按最窄条件降噪，不扩到所有融资新闻；Takeda海外药企反垄断/诉讼只按investing_news窄条件处理，不扩到所有医药新闻；suspicious_count=0 但 watchdog alert 时优先排查 failed_sources` |
