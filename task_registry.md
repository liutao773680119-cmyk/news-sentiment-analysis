...
| `global-multisource-mainline` | `全球多源主线 live report 尾噪收口` | `active` | `main` | `2026-05-01` | `tail -n 120 /tmp/news-sentiment-watch.log` | `不要再按旧 phase8 的 A股单线标准验收；audit=0 后仍要直接看 report 头部；report 过滤和 audit-suspicious 要同步；report 重写和读取不要并行；上一轮 log 块早于最新 commit 时不要误判规则未生效；HF Sinclair / AIG 先看多轮稳定性再决定是否处理` |
