...
| `global-multisource-mainline` | `全球多源主线 live report 尾噪收口` | `active` | `main` | `2026-04-29` | `sed -n '1,180p' data/reports/latest_report.txt` | `不要再按旧 phase8 的 A股单线标准验收；audit=0 后仍要直接看 report 头部；report 过滤和 audit-suspicious 要同步；当前头部已回到退市风险/真实并购样本，不要继续为了更短头部机械扩词；海外单股主题异动不要拉回 A股层` |
