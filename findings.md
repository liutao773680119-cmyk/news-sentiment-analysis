...
- 规则决策：
  - `audit-suspicious` 入口直接忽略所有 `ST/*ST` 标题。
  - `text_report` relevance 入口也直接忽略所有 `ST/*ST` 标题。
  - 只改输出和巡检，不改采集、事件分类、评分链路。
- 红灯样本：
  - `*ST西发：关于回复《深圳证券交易所对公司重大资产购买的问询函》的公告`
  - `ST中青宝：关于撤销其他风险警示暨股票停复牌的公告`
- 验收结论：
  - 新增 `audit` / `report` 红灯测试先复现漏出，再转绿。
  - 同步更新旧回归后，相关 `report` 定向组 `8 passed`。
  - 当前即时审计：`audit-suspicious --limit 20` -> `suspicious_count=0`
  - 当前即时报告：`latest_report.txt` 中 `ST/*ST` 关注项 -> `0`
- 风险结论：
  - 本轮是有意牺牲 `ST/*ST` 全量信息；之后如果用户问“为什么某个 ST 退市风险没看到”，这不是漏判，而是本轮规则结果。
