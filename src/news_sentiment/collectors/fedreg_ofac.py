...
FEDREG_OFAC_URL = (
    ...
    "?per_page=20&conditions%5Bagency_ids%5D%5B%5D=203&order=newest"
)


def fetch_fedreg_ofac_payload(url: str | None = None) -> dict:
    source_definition = load_source_definition_map()["fedreg_ofac"]
    payload = fetch_html(
        url or FEDREG_OFAC_URL,
    ...
    try:
        ...
        raise CollectorParseError("fedreg_ofac", f"invalid json payload: {exc}") from exc
...
def parse_fedreg_ofac_payload(payload: dict) -> list[RawNews]:
    results = payload.get("results")
    if not isinstance(results, list):
        raise CollectorParseError("fedreg_ofac", "invalid federal register payload")
    ...
    for item in results:
        ...
        rows.append(
            RawNews(
                news_id=f"fedreg_ofac-{document_number}",
                source="fedreg_ofac",
                source_type="policy",
...
def collect_fedreg_ofac_news() -> list[RawNews]:
    try:
        payload = fetch_fedreg_ofac_payload()
    except Exception as exc:
        raise CollectorFetchError("fedreg_ofac", str(exc) or exc.__class__.__name__) from exc

    if not payload:
        raise CollectorEmptyResultError("fedreg_ofac", "empty response body")

    rows = parse_fedreg_ofac_payload(payload)
    if not rows:
        raise CollectorParseError("fedreg_ofac", "no federal register items matched payload")
    return rows
