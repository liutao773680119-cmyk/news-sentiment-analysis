...
INVESTING_NEWS_FEED_URL = "https://www.investing.com/rss/news_25.rss"


def fetch_investing_news_feed(url: str | None = None) -> str:
    source_definition = load_source_definition_map()["investing_news"]
    return fetch_html(
        url or INVESTING_NEWS_FEED_URL,
...
def parse_investing_news_feed(payload: str) -> list[RawNews]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        raise CollectorParseError("investing_news", f"invalid rss payload: {exc}") from exc
    ...
    for index, item in enumerate(root.findall("./channel/item"), start=1):
        ...
        rows.append(
            RawNews(
                news_id=f"investing_news-{published_at.astimezone(timezone.utc).strftime('%Y%m%d%H%M%S')}-{index}",
...
def collect_investing_news() -> list[RawNews]:
    try:
        payload = fetch_investing_news_feed()
    except Exception as exc:
        raise CollectorFetchError("investing_news", str(exc) or exc.__class__.__name__) from exc

    if not payload.strip():
        raise CollectorEmptyResultError("investing_news", "empty response body")

    rows = parse_investing_news_feed(payload)
    if not rows:
        raise CollectorParseError("investing_news", "no feed items matched rss")
    return rows
