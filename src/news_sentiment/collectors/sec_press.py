...
SEC_PRESS_FEED_URL = "https://www.sec.gov/news/pressreleases.rss"


def fetch_sec_press_feed(url: str | None = None) -> str:
    source_definition = load_source_definition_map()["sec_press"]
    return fetch_html(
        url or SEC_PRESS_FEED_URL,
...
def parse_sec_press_feed(payload: str) -> list[RawNews]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        raise CollectorParseError("sec_press", f"invalid rss payload: {exc}") from exc
    ...
    for index, item in enumerate(root.findall("./channel/item"), start=1):
        ...
        rows.append(
            RawNews(
                news_id=f"sec_press-{published_at.astimezone(timezone.utc).strftime('%Y%m%d%H%M%S')}-{index}",
                source="sec_press",
                source_type="policy",
...
def collect_sec_press_news() -> list[RawNews]:
    try:
        payload = fetch_sec_press_feed()
    except Exception as exc:
        raise CollectorFetchError("sec_press", str(exc) or exc.__class__.__name__) from exc

    if not payload.strip():
        raise CollectorEmptyResultError("sec_press", "empty response body")

    rows = parse_sec_press_feed(payload)
    if not rows:
        raise CollectorParseError("sec_press", "no feed items matched rss")
    return rows
