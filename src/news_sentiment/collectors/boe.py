...
BOE_NEWS_FEED_URL = "https://www.bankofengland.co.uk/rss/news"
BOE_NEWS_PATH = "/news/"


def fetch_boe_news_feed(url: str | None = None) -> str:
    source_definition = load_source_definition_map()["boe"]
    return fetch_html(
        url or BOE_NEWS_FEED_URL,
...
def parse_boe_news_feed(payload: str) -> list[RawNews]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        raise CollectorParseError("boe", f"invalid rss payload: {exc}") from exc
    ...
    for index, item in enumerate(root.findall("./channel/item"), start=1):
        ...
        if not title or not link or BOE_NEWS_PATH not in link or not pub_date or link in seen_urls:
            continue
        seen_urls.add(link)
        published_at = _parse_pub_date(pub_date).replace(microsecond=0).isoformat()
        rows.append(
            RawNews(
                news_id=f"boe-{_parse_pub_date(pub_date).astimezone(timezone.utc).strftime('%Y%m%d%H%M%S')}-{index}",
                source="boe",
                source_type="policy",
...
def collect_boe_news() -> list[RawNews]:
    try:
        payload = fetch_boe_news_feed()
    except Exception as exc:
        raise CollectorFetchError("boe", str(exc) or exc.__class__.__name__) from exc

    if not payload.strip():
        raise CollectorEmptyResultError("boe", "empty response body")

    rows = parse_boe_news_feed(payload)
    if not rows:
        raise CollectorParseError("boe", "no news items matched rss")
    return rows
