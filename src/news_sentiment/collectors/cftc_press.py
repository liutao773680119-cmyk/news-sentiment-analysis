...
CFTC_PRESS_FEED_URL = "https://www.cftc.gov/RSS/RSSGP/rssgp.xml"


def fetch_cftc_press_feed(url: str | None = None) -> str:
    source_definition = load_source_definition_map()["cftc_press"]
    return fetch_html(
        url or CFTC_PRESS_FEED_URL,
...
def parse_cftc_press_feed(payload: str) -> list[RawNews]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        raise CollectorParseError("cftc_press", f"invalid rss payload: {exc}") from exc
    ...
    for index, item in enumerate(root.findall("./channel/item"), start=1):
        ...
        rows.append(
            RawNews(
                news_id=f"cftc_press-{published_at.astimezone(timezone.utc).strftime('%Y%m%d%H%M%S')}-{index}",
                source="cftc_press",
                source_type="policy",
...
def collect_cftc_press_news() -> list[RawNews]:
    try:
        payload = fetch_cftc_press_feed()
    except Exception as exc:
        raise CollectorFetchError("cftc_press", str(exc) or exc.__class__.__name__) from exc

    if not payload.strip():
        raise CollectorEmptyResultError("cftc_press", "empty response body")

    rows = parse_cftc_press_feed(payload)
    if not rows:
        raise CollectorParseError("cftc_press", "no feed items matched rss")
    return rows
