...
BOC_PRESS_FEED_URL = "https://www.bankofcanada.ca/content_type/press-releases/feed/"
RSS_NS = {
...
def fetch_boc_press_feed(url: str | None = None) -> str:
    source_definition = load_source_definition_map()["boc_press"]
    return fetch_html(
        url or BOC_PRESS_FEED_URL,
...
def parse_boc_press_feed(payload: str) -> list[RawNews]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as exc:
        raise CollectorParseError("boc_press", f"invalid rss payload: {exc}") from exc
    ...
    for index, item in enumerate(root.findall("rss:item", RSS_NS), start=1):
        ...
        rows.append(
            RawNews(
                news_id=f"boc_press-{_parse_pub_date(pub_date).astimezone(timezone.utc).strftime('%Y%m%d%H%M%S')}-{index}",
                source="boc_press",
                source_type="policy",
...
def collect_boc_press_news() -> list[RawNews]:
    try:
        payload = fetch_boc_press_feed()
    except Exception as exc:
        raise CollectorFetchError("boc_press", str(exc) or exc.__class__.__name__) from exc

    if not payload.strip():
        raise CollectorEmptyResultError("boc_press", "empty response body")

    rows = parse_boc_press_feed(payload)
    if not rows:
        raise CollectorParseError("boc_press", "no press release items matched rss")
    return rows
