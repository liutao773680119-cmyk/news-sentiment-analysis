...
def _should_merge(left: NormalizedNews, right: NormalizedNews) -> bool:
    if left.source in {"hkex", "fed", "ecb", "boe", "boc_press", "boj", "fedreg_sec", "fedreg_ofac", "bis", "sec_press", "cftc_press", "investing_news", "investing_forex", "investing_economic"} or right.source in {"hkex", "fed", "ecb", "boe", "boc_press", "boj", "fedreg_sec", "fedreg_ofac", "bis", "sec_press", "cftc_press", "investing_news", "investing_forex", "investing_economic"}:
...
def _is_industry_data_fast_news(text: str) -> bool:
    ...
    if _contains_any(text, ("零售价", "美元/加仑")) and _contains_any(
        text,
        ("汽油", "柴油"),
    ):
        return True
